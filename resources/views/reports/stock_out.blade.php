...
<div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-3 mb-4 btn-print-hide">
    <div>
        <h4 class="fw-bold mb-1">Laporan Rekapitulasi Stok Keluar</h4>
        <p class="text-muted small mb-0">Laporan data distribusi dan pengeluaran barang inventaris internal kantor pos</p>
    </div>
...
<div class="card mb-4 btn-print-hide">
    <div class="card-body p-3">
        <form method="GET" action="{{ route('reports.stock-out') }}" class="row g-2 align-items-center">
            <div class="col-md-3">
...
<div class="card p-4">
    <div class="border-bottom pb-3 mb-4 text-center">
        <h4 class="fw-bold text-uppercase mb-1">LAPORAN PENGELUARAN & DISTRIBUSI STOK KELUAR</h4>
        ...
        <table class="table table-bordered align-middle mb-0" style="font-size: 0.88rem;">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="font-monospace fw-bold text-danger">{{ $detail->stockOut->transaction_number ?? '-' }}</td>
                    <td class="text-nowrap">{{ $detail->stockOut->transaction_date ? $detail->stockOut->transaction_date->format('d/m/Y') : '-' }}</td>
                    ...
                    <td>{{ $detail->stockOut->user->name ?? '-' }}</td>
                    <td class="small text-muted">{{ $detail->stockOut->notes ?? '-' }}</td>
                </tr>
                @empty
                <tr>
                    <td colspan="8" class="text-center py-4 text-muted">Tidak ada transaksi stok keluar dalam periode ini.</td>
                </tr>
                @endforelse
            </tbody>
            <tfoot class="table-light fw-bold">
                <tr>
                    <td colspan="5" class="text-end">Total Jumlah Barang Keluar:</td>
                    <td class="text-center text-danger">-{{ number_format($details->sum('quantity')) }} unit</td>
                    <td colspan="2"></td>
                </tr>
            </tfoot>
        </table>
